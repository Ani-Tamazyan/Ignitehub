...
function NewsPart1({ onFilterChange }) {
  const handleIndustryClick = (industry) => {
    console.log('Industry Clicked:', industry);
    onFilterChange({ industry, region: null });
  };

  const handleRegionClick = (region) => {
    console.log('Region Clicked:', region);
    onFilterChange({ industry: null, region });
  };
        ...
        <div className='NewsPart1_left'>
          ...
          <div className='Industries'>
            ...
            <ul>
              {Industries.map(({ id, name }) => (
                <li key={id}>
                  <button onClick={() => handleIndustryClick(name)}>{name}</button>
                </li>
              ))}
            </ul>
          </div>
          <div className='Regions'>
            ...
            <ul>
              {Regions.map(({ id, name }) => (
                <li key={id}>
                  <button onClick={() => handleRegionClick(name)}>{name}</button>
                </li>
              ))}
            </ul>
          </div>
